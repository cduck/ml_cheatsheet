...
$f(\bx) = \tp{\bx}\bA\bB + \bA\bB\bx + \tp{\bx}\bA\bB\bx \Leftrightarrow \frac{df(\bx)}{d\bx} = \bA\bB + \bA\bB + 2\bA\bB\bx$
\ruler
$\bH_{i,j}=\frac{\partial^2 f}{\partial x_i \partial x_j}$;
$\nabla_{x}(a\bx) = a\bI$
\ruler
$\nabla_{x}x^T A x = x^T(A+A^T) = (A+A^T)x$
% --------------------------------
...
$\epsilon_{NN}(x)$: Error of bearest neighbor\\
Theorem: $\lim_{n\rightarrow\infty}\epsilon_{NN}\leq 2\epsilon^*$\\
For kNN: $\lim_{n\rightarrow\infty, k\rightarrow\infty,k/n\rightarrow\infty}\epsilon_{kNN}=\epsilon^*$
\ruler
\textbf{Curse of Dimensionality}:\\
Problem: D on the order of (or greater than) N\\
Remedies:\\
\begin{enumerate} \itemsep0pt \parskip0pt \parsep0pt
  \item Get more data: Make N larger
  \item Make better features: Make D smaller
  \item Use better distance metric: Effectively make D smaller ☺︎
\end{enumerate}
